<?php
//Include Common Files @1-1EBBCDBB
define("RelativePath", ".");
define("PathToCurrentPage", "/");
define("FileName", "DiputadoResumen.php");
...
//Open Method @92-E88189F6
    function Open()
    {
        ...
        $this->SQL = "EXEC spLegislativoNacionalTally " . ";";
...
class clsRecordSelectForm { //SelectForm Class @130-0E16A801

//Variables @130-9E315808
...
//Class_Initialize Event @130-F8E37BA5
    function clsRecordSelectForm($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->circunscripcion = new clsControl(ccsListBox, "circunscripcion", "circunscripcion", ccsText, "", CCGetRequestParam("circunscripcion", $Method, NULL), $this);
            $this->circunscripcion->DSType = dsSQL;
            $this->circunscripcion->DataSource = new clsDBmssql();
            $this->circunscripcion->ds = & $this->circunscripcion->DataSource;
            list($this->circunscripcion->BoundColumn, $this->circunscripcion->TextColumn, $this->circunscripcion->DBFormat) = array("circunscipcion", "nombre", "");
            $this->circunscripcion->DataSource->SQL = "select convert(varchar(2),c.cod_prov)+convert(varchar(1),c.circ) circunscipcion ,p.nombre + ' 0' + convert(varchar(1),c.circ) nombre\n" .
            "from circuitos c\n" .
            "inner join provincias p on p.cod_prov=c.cod_prov {SQL_OrderBy}";
            $this->circunscripcion->DataSource->Order = "2";
...
//Initialize Method @130-5D060BAC
    function Initialize()
    {
...
//Validate Method @130-CEAB173C
    function Validate()
    {
        global $CCSLocales;
        $Validation = true;
        $Where = "";
        $Validation = ($this->circunscripcion->Validate() && $Validation);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "OnValidate", $this);
        $Validation =  $Validation && ($this->circunscripcion->Errors->Count() == 0);
        return (($this->Errors->Count() == 0) && $Validation);
    }
//End Validate Method

//CheckErrors Method @130-683A83E7
    function CheckErrors()
    {
        $errors = false;
        $errors = ($errors || $this->circunscripcion->Errors->Count());
...
//Operation Method @130-E974C8E4
    function Operation()
    {
        ...
        $Redirect = "DiputadoDetalle.php" . "?" . CCGetQueryString("All", array("ccsForm"));
...
//Show Method @130-282F4972
    function Show()
    {
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            $Error = "";
            $Error = ComposeStrings($Error, $this->circunscripcion->Errors->ToString());
...
} //End SelectForm Class @130-FCB6E20C

class clsSelectFormDataSource extends clsDBmssql {  //SelectFormDataSource Class @130-22532594

//DataSource Variables @130-55C4D737
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
...
//DataSourceClass_Initialize Event @130-BECFB051
    function clsSelectFormDataSource(& $Parent)
    {
        $this->Parent = & $Parent;
        $this->ErrorBlock = "Record SelectForm/Error";
        $this->Initialize();
        $this->circunscripcion = new clsField("circunscripcion", ccsText, "");
        

    }
//End DataSourceClass_Initialize Event

//Prepare Method @130-14D6CD9D
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
...
//SetValues Method @130-BAF0975B
    function SetValues()
    {
    }
//End SetValues Method

} //End SelectFormDataSource Class @130-FCB6E20C
...
//Open Method @77-C15307E9
    function Open()
    {
        $this->cp["RETURN_VALUE"] = new clsSQLParameter("urlRETURN_VALUE", ccsInteger, "", "", CCGetFromGet("RETURN_VALUE", NULL), "", false, $this->ErrorBlock);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "EXEC spDiputadosResumenGeneral " . ";";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteSelect", $this->Parent);
        $this->query($this->SQL);
        $this->RecordsCount = "CCS not counted";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterExecuteSelect", $this->Parent);
...
//Open Method @86-AF67C282
    function Open()
    {
        $this->cp["RETURN_VALUE"] = new clsSQLParameter("urlRETURN_VALUE", ccsInteger, "", "", CCGetFromGet("RETURN_VALUE", NULL), "", false, $this->ErrorBlock);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "EXEC spLegislativoNacionalPart " . ";";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteSelect", $this->Parent);
        $this->query($this->SQL);
        $this->RecordsCount = "CCS not counted";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterExecuteSelect", $this->Parent);
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "DiputadoResumen.html";
$BlockToParse = "main";
...
//Go to destination page @1-474E292D
if($Redirect)
{
