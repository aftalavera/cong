...
//Connection Settings @0-42233AEB
...
//Initialize Common Variables @0-D7715166
...
//intranet Connection Class @-94AFDA4D
class clsDBintranet extends DB_Adapter
{
    function clsDBintranet()
    {
        $this->Initialize();
    }
    ...
    function Initialize()
    {
        ...
        $this->SetProvider($CCConnectionSettings["intranet"]);
        parent::Initialize();
...
//End intranet Connection Class

//mssql Connection Class @-72FD7AEC
class clsDBmssql extends DB_Adapter
{
    function clsDBmssql()
    {
        $this->Initialize();
    ...
    {
        global $CCConnectionSettings;
        $this->SetProvider($CCConnectionSettings["mssql"]);
        parent::Initialize();
        $this->DateLeftDelimiter = "'";
        $this->DateRightDelimiter = "'";
    }
...
//CCLoginUser @0-6455232E
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBintranet();
    $SQL = "SELECT emp_id, group_id, emp_password FROM employees WHERE emp_login=" . $db->ToSQL($login, ccsText) . " AND emp_password=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
