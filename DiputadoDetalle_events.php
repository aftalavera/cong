<?php
//Page_BeforeInitialize @1-1558D4B4
function Page_BeforeInitialize(& $sender)
{
    $Page_BeforeInitialize = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $DiputadoDetalle; //Compatibility
//End Page_BeforeInitialize

//FlashChart1 Initialization @136-80C2C1E4
    if ('DiputadoDetalleFlashChart1' == CCGetParam('callbackControl')) {
        global $CCSLocales;
        $Service = new Service();
        $formatter = new TemplateFormatter();
        $formatter->SetTemplate(file_get_contents(RelativePath . "/" . "DiputadoDetalleFlashChart1.xml"));
        $Service->SetFormatter($formatter);
//End FlashChart1 Initialization

//FlashChart1 DataSource @136-05BF182C
        ...
        $Service->DataSource->cp["circ"] = new clsSQLParameter("urlcircunscripcion", ccsInteger, "", "", CCGetFromGet("circunscripcion", NULL), 11, false, $Service->DataSource->ErrorBlock);
        $Service->DataSource->SQL = "EXEC spDiputadosDetalleGrafica " . $Service->DataSource->ToSQL($Service->DataSource->cp["circ"]->GetDBValue(), $Service->DataSource->cp["circ"]->DataType) . ";";
